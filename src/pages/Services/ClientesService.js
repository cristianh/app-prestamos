export default class ClientesService {
    ...
    actualizarPosicionCliente(id_admin, id_empresa, id_cobrador, id_cliente, data) {
        return axios.post(`actualizarPosicionClienteLista?idadmin=${id_admin}&id_empresa=${id_empresa}&doc=${id_cobrador}&subdoc=${id_cliente}`, data).then(response => response);
    }
    ...
    actualizarClienteCobrador(id_admin, id_empresa, id_cobrador, id_cliente, data) {
        return axios.post(`CobradoresClientesUpdate?idadmin=${id_admin}&id_empresa=${id_empresa}&doc=${id_cobrador}&subdoc=${id_cliente}`, data).then(response => response);
    }

    guardarObservacionNoPago(id_admin, id_empresa, ui_cobrador, id_cliente, data) {
        return axios.post(`CobradoresGuardarObservacionNoPago?idadmin=${id_admin}&id_empresa=${id_empresa}&doc=${ui_cobrador}&sub=${id_cliente}`, data).then(response => response);
    }

    eliminarPrestamoPago(id_admin, id_empresa, ui_cobrador, id_cliente, data) {
        return axios.post(`ClienteEliminarPrestamosPago?idadmin=${id_admin}&id_empresa=${id_empresa}&doc=${ui_cobrador}&sub=${id_cliente}`, data).then(response => response);
    }
